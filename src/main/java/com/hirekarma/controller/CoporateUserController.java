...
//					BeanUtils.copyProperties(userProfileReturn, userBean);
				...
				throw new CoporateUserDefindException("Please Enter Valid Email address");
			}
...
//	}
	...
	public ResponseEntity<?> updateCoporateUserProfile(@ModelAttribute UserBean bean) {
		...
		try {
			...
			if (Validation.validateEmail(bean.getEmail())) {
				if (Validation.phoneNumberValidation(Long.valueOf(bean.getPhoneNo()))) {
					...
					if (userBean != null) {
						...
						return new ResponseEntity<>(new Response("success",404,"No such user found",null,null), HttpStatus.NOT_FOUND);
					}
				} else {
			...
			LOGGER.error("Some problem occured in CoporateUserController.updateCoporateUserProfile(-): " + e);
			e.printStackTrace();
			return new ResponseEntity<>(new Response("Error",500,e.getMessage(),null,null), HttpStatus.INTERNAL_SERVER_ERROR);
		}
	}
	...
	public ResponseEntity<Response> findCorporateById(@PathVariable Long corpUserId) {
		...
		try {
			...
			if (userBean != null) {
				...
				userBean.setPassword(null);
				return new ResponseEntity<Response>(new Response("Success",HttpStatus.OK,"",userBean,null), HttpStatus.OK);
			} else {
				LOGGER.info("Coporate details not found in CoporateUserController.findCorporateById(-)");
				return new ResponseEntity<Response>(new Response("Error",HttpStatus.NOT_FOUND,"No such entity found",null,null), HttpStatus.NOT_FOUND);
			}
		} catch (Exception e) {
			LOGGER.error("Some problem occured in CoporateUserController.findCorporateById(-): " + e);
			e.printStackTrace();
			return new ResponseEntity<Response>(new Response("Error",HttpStatus.INTERNAL_SERVER_ERROR,"something went wrong!",null,null), HttpStatus.INTERNAL_SERVER_ERROR);
		}
	}
